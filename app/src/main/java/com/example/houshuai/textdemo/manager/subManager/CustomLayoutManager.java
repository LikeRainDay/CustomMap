...
/**
 * @author 候帅
 *         Created by 候帅 on 2017/2/7. 14:04
 */
...
public abstract class CustomLayoutManager extends RecyclerView.LayoutManager {
    ...
    protected int mDecoratedChildHeiht;
    ...
    /**
     * You can set up yout own properties  here  or chang the exist
     * properties  like startLeft and startTop
     */
    ...
    public CustomLayoutManager(boolean isClockWise, Context context) {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (getChildCount() == 0) {
            ...
            //获取视图所使用的宽高
            mDecoratedChildHeiht = getDecoratedMeasuredHeight(scrap);
            mDecoratedChildWidth = getDecoratedMeasuredWidth(scrap);
            startLeft = (getHorizontalSpace() - mDecoratedChildWidth) / 2;
            startTop = (getVertalSpace() - mDecoratedChildHeiht) / 2;
    ...
    private void layoutItems(RecyclerView.Recycler recycler, RecyclerView.State state, int oriention) {
        ...
        int begin = getCurrentPosition() - MAX_DISPLAY_ITEM_COUNT;
        int end = getCurrentPosition() + MAX_DISPLAY_ITEM_COUNT;
        if (begin < 0) begin = 0;
    ...
    private void resetViewProperty(View scrap) {
        scrap.setRotation(0);
        scrap.setRotationY(0);
        scrap.setRotationX(0);
        scrap.setAlpha(1f);
    }
    ...
    private void layoutScrap(View scrap, float delta) {
        int left = calItemLeftPosition(delta);
        int top = calItemTopPosition(delta);
        layoutDecorated(scrap, startLeft + left, startTop + top, startLeft + left + mDecoratedChildWidth, startTop + top + mDecoratedChildHeiht);
        setItemViewProperty(scrap, delta);
    }

    private int calItemTopPosition(float delta) {
        return 0;
    }

    private int calItemLeftPosition(float delta) {
        return (int) delta;
    }

    private float propertyChangeWhenScroll(View scrap) {
        return scrap.getLeft() - startLeft;
    }
